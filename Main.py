...
async def on_message(msg):
    if msg.server and msg.channel.name == 'tanki_online_test' and msg.author.id != bot.user.id:
		...
		if msg.content.startswith("d!c open"):
		...
		if msg.content.upper().startswith("NOOB DYNO"):
		...
		if msg.content.upper().startswith('D!8BALL'):
		...
		if msg.content.upper().startswith('HELLO BOT'):
			msgs = "hello {0.author.mention}".format(message)
			await bot.send_message(message.channel, msgs)
		if msg.content.startswith('d!hello'):
			msgs = "hello {0.author.mention}".format(message)
			await bot.send_message(message.channel, msgs)
